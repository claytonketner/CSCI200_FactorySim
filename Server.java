...
/**
 * class implementing a server application to coordinate factory clients over a
 * network (the server-side factory control window is implemented in the
 * FactoryControlManager class)
 */
public class Server implements ActionListener, Networked {
	...
	public Server() throws IOException {
		...
		// load factory settings from file (or set up new factory if can't load
		// from file)
		loadSettings();
	...
	/**
	 * called during timer tick; updates simulation and broadcasts factoryUpdate
	 * to clients
	 */
		...
		if (ae.getSource() instanceof javax.swing.Timer) {
			...
			for (Map.Entry<Integer, GUIItem> e : state.items.entrySet()) {
				...
				if (e.getValue() instanceof GUIKitCamera) {
					...
					if (kitCamera.isExpired(update.timeElapsed))
						update.removeItems.add(key);
					...
					if (kitRobot.arrived(update.timeElapsed)) {
						Point2D.Double target = new Point2D.Double(
								kitRobot.getBasePos().x + Math.random() * 200
										- 100, kitRobot.getBasePos().y
										+ Math.random() * 200 - 100);
						update.itemMoves.put(key, kitRobot.movement
								.moveToAtSpeed(update.timeElapsed, target, 0,
										100));
					}
					...
					if (partRobot.arrived(update.timeElapsed)) {
						Point2D.Double target = new Point2D.Double(
								partRobot.getBasePos().x + Math.random() * 200
										- 100, partRobot.getBasePos().y
										+ Math.random() * 200 - 100);
						update.itemMoves.put(key, partRobot.movement
								.moveToAtSpeed(update.timeElapsed, target, 0,
										100));
					}
				}
	...
	public void msgReceived(Object msgObj, NetComm sender) {
		...
		if (senderIndex == netComms.size()) {
			System.out
					.println("Warning: received message from unknown client: "
							+ msgObj);
			return;
		}
		// handle message
		if (msgObj instanceof CloseConnectionMsg) {
			...
			// (but don't call clients.get(i).close() because client might still
			// receive the message and get confused)
			...
			// broadcast message to all clients (for TestClient only, TODO:
			// delete later)
			for (int i = 0; i < netComms.size(); i++) {
				netComms.get(i).write(
						"Message from " + senderIndex + " to " + i + ": "
								+ (String) msgObj);
			}
			...
			if (addPart(senderIndex, (NewPartMsg) msgObj, true)) {
				...
				System.out.println("Client " + senderIndex
						+ " unsuccessfully tried to add a part");
			}
			...
			if (changePart(senderIndex, (ChangePartMsg) msgObj)) {
				...
				System.out.println("Client " + senderIndex
						+ " unsuccessfully tried to change a part");
			}
			...
			if (deletePart(senderIndex, (DeletePartMsg) msgObj, true) != null) {
				...
				System.out.println("Client " + senderIndex
						+ " unsuccessfully tried to delete a part");
			}
			...
			if (addKit(senderIndex, (NewKitMsg) msgObj, true)) {
				...
				System.out.println("Client " + senderIndex
						+ " unsuccessfully tried to add a kit");
			}
			...
			if (changeKit(senderIndex, (ChangeKitMsg) msgObj)) {
				...
				System.out.println("Client " + senderIndex
						+ " unsuccessfully tried to change a kit");
			}
			...
			if (deleteKit(senderIndex, (DeleteKitMsg) msgObj, true) != null) {
				...
				System.out.println("Client " + senderIndex
						+ " unsuccessfully tried to delete a kit");
			}
			...
			if (produceKits(senderIndex, (ProduceKitsMsg) msgObj)) {
				System.out.println("Client " + senderIndex
						+ " added a production request");
			} else {
				System.out.println("Client " + senderIndex
						+ " unsuccessfully tried to add a production request");
			}
			...
			System.out.println("Sent production status to client "
					+ senderIndex);
			...
			System.out.println("Warning: received unknown message from client "
					+ senderIndex + ": " + msgObj);
		}
	}

	/**
	 * adds part to partTypes (if valid), if notify is true sends StringMsg to
	...
	private boolean addPart(int clientIndex, NewPartMsg msg, boolean notify) {
		...
		if (notify) {
			netComms.get(clientIndex).write(
					new StringMsg(StringMsg.MsgType.NEW_PART, valid));
		}
		if (!valid.isEmpty())
			return false;
	...
	/**
	 * changes specified part (if valid and not in production), sends StringMsg
	 * to client indicating success or failure
	 */
	private boolean changePart(int clientIndex, ChangePartMsg msg) {
		// delete old part
		Part oldPart = deletePart(clientIndex,
				new DeletePartMsg(msg.oldNumber), false);
		if (oldPart == null) {
			netComms.get(clientIndex)
					.write(new StringMsg(StringMsg.MsgType.CHANGE_PART,
							"Requested part either in production or does not exist"));
		}
		// add replacement part
		else if (!addPart(clientIndex, new NewPartMsg(msg.part), false)) {
			netComms.get(clientIndex).write(
					new StringMsg(StringMsg.MsgType.CHANGE_PART,
							newPartIsValid(msg.part)));
	...
	/**
	 * deletes part with specified name (if exists), if notify is true sends
	 * StringMsg to client indicating success or failure, returns deleted part
	...
	private Part deletePart(int clientIndex, DeletePartMsg msg, boolean notify) {
		...
		for (i = 0; i < status.cmds.size(); i++) {
			if (status.status.get(i) == ProduceStatusMsg.KitStatus.QUEUED
				...
				for (j = 0; j < kit.parts.size(); j++) {
					if (msg.number == kit.getPart(j).getNumber()) {
						if (notify)
							netComms.get(clientIndex)
									.write(new StringMsg(
											StringMsg.MsgType.DELETE_PART,
											"May not delete part that is in production"));
						System.out.println("May not delete part that is in production");
						return null;
					}
				}
			}
		...
		for (i = 0; i < partTypes.size(); i++) {
			if (msg.number == partTypes.get(i).getNumber()) {
				...
				if (notify) {
					netComms.get(clientIndex).write(
							new StringMsg(StringMsg.MsgType.DELETE_PART, ""));
					broadcast(WantsEnum.PART_TYPES);
				}
				return ret;
			}
		...
		if (notify)
			netComms.get(clientIndex).write(
					new StringMsg(StringMsg.MsgType.DELETE_PART,
							"Part never existed or has already been deleted"));
		return null;
	}

	/**
	 * adds kit to kitTypes (if valid), if notify is true sends StringMsg to
	 * client indicating success or failure
	 */
	private boolean addKit(int clientIndex, NewKitMsg msg, boolean notify) {
		...
		if (notify) {
			netComms.get(clientIndex).write(
					new StringMsg(StringMsg.MsgType.NEW_KIT, valid));
		}
		if (!valid.isEmpty())
			return false;
	...
	/**
	 * changes specified kit (if valid and not in production), sends StringMsg
	 * to client indicating success or failure
	 */
	private boolean changeKit(int clientIndex, ChangeKitMsg msg) {
		// delete old kit
		Kit oldKit = deleteKit(clientIndex, new DeleteKitMsg(msg.oldNumber),
				false);
		if (oldKit == null) {
			netComms.get(clientIndex)
					.write(new StringMsg(StringMsg.MsgType.CHANGE_KIT,
							"Requested kit either in production or does not exist"));
		}
		// add replacement kit
		else if (!addKit(clientIndex, new NewKitMsg(msg.kit), false)) {
			netComms.get(clientIndex).write(
					new StringMsg(StringMsg.MsgType.CHANGE_KIT,
							newKitIsValid(msg.kit)));
			kitTypes.add(oldKit);
		} else {
			netComms.get(clientIndex).write(
					new StringMsg(StringMsg.MsgType.CHANGE_KIT, ""));
	...
	/**
	 * deletes kit with specified name (if exists), if notify is true sends
	 * StringMsg to client indicating success or failure, returns deleted kit if
	...
	private Kit deleteKit(int clientIndex, DeleteKitMsg msg, boolean notify) {
		...
		for (i = 0; i < status.cmds.size(); i++) {
			if (status.status.get(i) == ProduceStatusMsg.KitStatus.QUEUED
					&& status.cmds.get(i).kitNumber == msg.number
					|| status.status.get(i) == ProduceStatusMsg.KitStatus.PRODUCTION
					&& status.cmds.get(i).kitNumber == msg.number) {

				netComms.get(clientIndex)
						.write(new StringMsg(StringMsg.MsgType.DELETE_KIT,
								"May not delete kit that is in production or in queued"));
				System.out.println("May not delete kit that is in production");
				return null;
			}
		}
		...
		/*
		 * for (i = 0; i < status.cmds.size(); i++) { if (status.status.get(i)
		 * == ProduceStatusMsg.KitStatus.QUEUED || status.status.get(i) ==
		 * ProduceStatusMsg.KitStatus.PRODUCTION) { Kit kit =
		 * getKitByNumber(status.cmds.get(i).kitNumber); for (j = 0; j <
		 * kit.partsNeeded.size(); j++) { if (msg.number ==
		 * kit.partsNeeded.get(j).getNumber()) { if (notify)
		 * netComms.get(clientIndex).write(new
		 * StringMsg(StringMsg.MsgType.DELETE_PART,
		 * "May not delete part that is in production")); return null; } } } }
		 */
		...
		for (i = 0; i < kitTypes.size(); i++) {
			if (msg.number == kitTypes.get(i).getNumber()) {
				...
				if (notify) {
					netComms.get(clientIndex).write(
							new StringMsg(StringMsg.MsgType.DELETE_KIT, ""));
					broadcast(WantsEnum.KIT_TYPES);
				}
				return ret;
			}
		...
		if (notify)
			netComms.get(clientIndex).write(
					new StringMsg(StringMsg.MsgType.DELETE_KIT,
							"Kit never existed or has already been deleted"));
		return null;
	}

	/**
	 * returns empty string if given part is valid, or error message if it is
	 * not
	 */
	...
	private String newKitIsValid(Kit kit) {
		if (kit.getNumber() <= 0)
			return "Kit number must be a positive integer";
		if (!isValidName(kit.getName())) {
			if (kit.getName().isEmpty())
		...
		// number of parts in kit validated on client side, so don't need to
		// check it here
		return "";
	}

	/**
	 * queue specified production command in production status (if valid), sends
	 * StringMsg to client indicating success or failure
	 */
	private boolean produceKits(int clientIndex, ProduceKitsMsg msg) {
		if (msg.howMany <= 0) {
			netComms.get(clientIndex).write(
					new StringMsg(StringMsg.MsgType.PRODUCE_KITS,
							"Must produce at least 1 new kit"));
			return false;
		}
		if (getKitByNumber(msg.kitNumber) == null) {
			netComms.get(clientIndex).write(
					new StringMsg(StringMsg.MsgType.PRODUCE_KITS,
							"Kit number must refer to an existing kit"));
			return false;
		}
		status.cmds.add(msg);
	...
	public void broadcast(WantsEnum wantsEnum) {
		for (int i = 0; i < wants.size(); i++) {
			...
			} else if (wantsEnum == WantsEnum.KIT_TYPES
					&& wants.get(i).kitTypes) {
	...
	/**
	 * returns part type with specified part number, or null if there is no such
	 * part
	 */
	private Part getPartByNumber(int number) {
		for (int i = 0; i < partTypes.size(); i++) {
			if (partTypes.get(i).getNumber() == number)
	...
	/**
	 * returns kit type with specified kit number, or null if there is no such
	 * kit
	 */
	private Kit getKitByNumber(int number) {
		for (int i = 0; i < kitTypes.size(); i++) {
			if (kitTypes.get(i).getNumber() == number)
				return kitTypes.get(i);
		}
		return null;
	}

	/**
	 * returns whether specified part/kit name is valid (i.e. is not empty and
	 * is composed only of letters, numbers, or spaces); copied from Andrew's
	 * HW3 submission
	 */
	...
	private void initFactory() {
		...
		for (int i = 0; i < 4; i++) {
			...
			GUILane guiLane = new GUILane(new Lane(), true, 6, 630, 124
					+ laneSeparation * i);
			guiLane.turnOff(0);
			...
			state.add(new GUIFeeder(new Feeder(), 1165, 170 + laneSeparation
					* i));
		...
		GUIKitDeliveryStation guiKitDeliv = new GUIKitDeliveryStation(
				new KitDeliveryStation(), new GUILane(new Lane(), false, 8,
						350, -10), new GUILane(new Lane(), false, 3, 350 - 180,
						-10), 10, 10);
		...
		state.add(new GUIPartRobot(new PartRobot(),
				new Point2D.Double(350, 340)));
		partRobotID = state.items.lastKey();
		GUIGantry guiGantry = new GUIGantry(100, 100);
		guiGantry.movement = guiGantry.movement.moveToAtSpeed(0,
				new Point2D.Double(500, 500), 0, 50);
		guiGantry.addBin(new GUIBin(new GUIPart(new Part(), 0, 0), new Bin(
				new Part(), 10), 0, 0));
		state.add(guiGantry);
	...
	private void loadSettings() {
		...
		try {
			...
			System.out
					.println("Settings file not found; a new factory has been set up.");
		} catch (Exception ex) {
			initFactory();
			System.out
					.println("Error loading settings from file; a new factory has been set up.");
		}
	}
		...
		public void run() {
			while (true) { // loop exits when user presses ctrl+C
				try {
					...
					System.out.println("Client " + (netComms.size() - 1)
							+ " has joined");
				} catch (Exception ex) {
